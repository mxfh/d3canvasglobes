...
// TODO: Sun Terminator
// TODO: Tissots-Indicatrices (Pseudo/real)


function globeOverlay() {
    var element, globe, land, coastlines, borders, lakes, fillColorA, fillColorB, textColor,
        width, height, origin, minSize, maxDim, minDim, canvasPadding, globePadding, colWidth, rowHeight,
        padding, gutter, baselineOffset, scales, scale, topoFile, clipAngle, diagonal, zoomMin, zoomMax,
        λA, φA, γA, λB, φB, γB, rotation, projection, canvas, context, path,
        posX, posY, rInit, r, x, y, xTmp, yTmp, xRel, yRel, mouseFactor, momentum, flag, shiftFlag, altFlag,
        toggleGradient, switchColors, showGlobeB, showBorders, showLakes, showHelp, showCoastlines, gradient, presets;
    ...
    if (width <= height) { minDim = width;  maxDim = height;}
    else {  minDim = height; maxDim = width;}
...
function handle(delta) {
    ...
    r = r + delta*(r/10);
    if (r >= diagonal / 2) {clipAngle = 90 - Math.acos((diagonal/2)/r)*180/Math.PI;}
    else  {clipAngle = 89}
    if (r > diagonal && r <= diagonal * 4 && scale != 1 ) {toggleGradient = 1; scale = 1; momentum = 0; loadGeometry(scales[scale]);}
    if (r > diagonal * 4 && scale != 2) {toggleGradient = 1; scale = 2; momentum = 0; loadGeometry(scales[scale]);}
    if (r <= diagonal) {toggleGradient = 0; initGradient(); scale = 0;  momentum = 1; loadGeometry(scales[scale]);}
...
function stopTrack() {
        ...
        if (momentum === 1){
            var refreshIntervalId = setInterval(function(){
                if (momentum === 0 ) {clearInterval(refreshIntervalId);}
                var linSlowdown = clipAngle*0.002;
                var multiSlowdown = 0.98;
                 if (xRel > 0) {xRel = (xRel) * multiSlowdown - linSlowdown;}
                 if (xRel < 0) {xRel = (xRel) * multiSlowdown + linSlowdown;}
                 if (yRel > 0) {yRel = (yRel) * multiSlowdown - linSlowdown;}
                 if (yRel < 0) {yRel = (yRel) * multiSlowdown + linSlowdown;}
                 rotate();
                 drawglobes(λA,φA,γA,λB,φB,γB);
                if (Math.abs(xRel) < 1 && Math.abs(yRel) < 1) {clearInterval(refreshIntervalId);}
             },1000/48);
        }
    }
function toggle(p){
...
function renderPositionInfo() {
    ...
    if (x !== undefined) {context.fillText("x :", getX(0), getYtext(0));}
    if (y !== undefined) {context.fillText("y :", getX(0), getYtext(1));}
    ...
    if (γA !== 0) {context.fillText("γ :", getX(1), getYtext(2)); }
    if (γB !== 0) {context.fillText("γ :", getX(2), getYtext(2)); }
    context.textAlign = "right";
    if (x !== undefined) {context.fillText(x, getXalignRight(0), getYtext(0));}
    if (y !== undefined) {context.fillText(y, getXalignRight(0), getYtext(1));}
    ...
    if (γA !== 0) {context.fillText(Math.round(γA), getXalignRight(1), getYtext(2)); }
    if (γB !== 0) {context.fillText(Math.round(γB), getXalignRight(2), getYtext(2)); }
